...
date: "`r format(lubridate::ymd('20180402'), '%B %d, %Y')`"
params:
  show_code: FALSE
  blue_color: "#5499C7"
  green_color: "#1ABC9C"
...
- $H_0$ is rejected in favor to $H_1$ (i.e. we conclude "_**there is a statistically significant difference between treatments E and C**_") at 5% significance level (2-sided), if $|t| > t_{0.975}(n_E+n_C-2)$, where $t_{0.975}(n_E+n_C-2)$  is a 97.5%-percentile of the $t$ distribution with $n_E+n_C-2$ degrees of freedom.
 
---

# How Large Should be Our Study?
...
Assuming equal $(1:1)$ allocation, the _**sample size per treatment arm**_ $(n)$ is determined to satisfy two major conditions:


$$\alpha = \text{Pr}\left(\text{Reject }H_0|\tau = 0\right)\text{  and  }1-\beta = \text{Pr}\left(\text{Reject }H_0|\tau = \Delta\right).$$
--

The _**sample size per treatment arm**_ is:
...
.pull-right[
- _**Adaptive design**_ -- a clinical study design that uses accumulating data to decide how to modify aspects of the study as it continues, without undermining the _integrity_ and _validity_ of the trial.
...
- Unblinding of interim data can introduce operational bias.
...
knitr::include_graphics('./figures/lecture01-fig05-fda2010.png')
```
...
![](./figures/lecture01-fig06-fda2016.png)
]
...
Phase I was excluded from the review; _**the majority of ADs are in Phase II trials**_ (which is reflective of regulatory guidance).

_**Group Sequential Design**_ (GSD) is _**most popular**_ type of AD in _**Phase III**_.
...
knitr::include_graphics('./figures/lecture01-fig09-classification-of-ad.png')
```
...
- _**Primary goal**_: evaluate clinically safety and PK; in oncology -- evaluate dose-toxicity relation and establish MTD.
]
...
- _**Continual Reassessment Method (CRM)**_ developed by O'Quigley et al. (1990)<sup>5</sup> and its numerous extensions.
]

.footnote[
[5] O'Quigley J, Pepe M, Fisher L (1990)  "_Continual reassessment method: a practical design for phase I clinical studies in cancer_." _Biometrics_, **27**, 33–48.
]
...
# MCM-Mod Methodology<sup>6</sup>

Combines multiple comparisons (MCP) with dose-response modeling (Mod).

```{r, echo=FALSE, out.width='60%', fig.align='center'}
knitr::include_graphics('./figures/lecture01-fig10-mcp-mod.png')
```

.footnote[
[6] Bretz F, Pinheiro JC, Branson M (2005)  "_Combining multiple comparisons and modeling techniques in dose-response studies_." _Biometrics_, **61**, 738–748.
]
...
knitr::include_graphics('./figures/lecture01-fig11-chmp.png')
```
...
.left-column[
![](./figures/lecture01-fig09-exploratory-03.png)
]
...
knitr::kable(rnd_table, "html", col.names = c("Randomization", "Purpose")) %>%
  kable_styling(bootstrap_options = "striped", font_size = 16) %>%
  row_spec(c(0, 2, 4), bold = TRUE, color = "white", background = params$blue_color) %>%
  row_spec(c(1, 3), bold = TRUE, color = "white", background = params$green_color)
```
...
- Skewed allocation (favoring more variable arms) can lead to more accurate estimates or more powerful tests.
...
Adaptive, dose-finding phase II trial evaluating the safety and efficacy of ABT-089 in patients with _**mild to moderate Alzheimer disease**_<sup>7</sup>.
...
.footnote[
[7] Lenz RA, Pritchett YL, Berry SM et al (2015)  "_Adaptive, dose-finding phase 2 trial evaluating the safety and efficacy of ABT-089 in mild to moderate Alzheimer disease_." _Alzheimer Disease and Associated Disorders_, **29**(3), 192–199.
]
...
- Subsequently randomization probabilities to ABT-089 groups were calculated adaptively (updates made at 2-week intervals), based on accumulating ADAS-Cog data, to increase allocation to doses viewed empirically as _**MED**_ and _**ED90**_.
...
knitr::include_graphics('./figures/lecture01-fig12-rar-example.png')
```
...
![](./figures/lecture01-fig13-dose-selection.png)
...
knitr::include_graphics('./figures/lecture01-fig14-seamless-trial.png')
```
...
.footnote[
Schematic taken from McEntegart D, Nicholls G, Byrom B (2007) "_Blinded by science with adaptive designs_." _Applied Clinical Trials_.
]
...
knitr::include_graphics('./figures/lecture01-fig15-seamless-paper01.png')
```
...
```{r, echo=FALSE, out.width='110%', fig.align='center'}
knitr::include_graphics('./figures/lecture01-fig15-seamless-paper02.png')
```
...
```{r, echo=FALSE, out.width='110%', fig.align='center'}
knitr::include_graphics('./figures/lecture01-fig16-confirmatory-paper01.png')
```
...
![](./figures/lecture01-fig17-confirmatory-01.png)
...
- CSDs are _**well accepted by regulators**_:
  ...
  + _"Adaptive Designs which properties are well understood"_.
]
...
![](./figures/lecture01-fig17-confirmatory-02.png)
...
![](./figures/lecture01-fig17-confirmatory-03.png)
...
```{r, echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('./figures/lecture01-fig18-decision-at-IA.png')
```
...
Schematic is taken from Maca J, Dragalin V, Gallo P(2014)  "_Adaptive clinical trials: overview of phase III designs and challenges_." _Therapeutic Innovation and Regulatory Science_, 
**48**(1), 31–40.
...
tribble(
  ...
  row_spec(c(0, 2, 4, 6), bold = TRUE, color = "white", background = params$blue_color) %>%
  row_spec(c(1, 3, 5, 7), bold = TRUE, color = "white", background = params$green_color)
```

---

# Analogy Between Adaptive Designs and Swiss Army Knife<sup>8</sup>

.left-column[

```{r, echo=FALSE, out.width='80%', fig.align='left'}
knitr::include_graphics('./figures/lecture01-fig19-scissor.png')
knitr::include_graphics('./figures/lecture01-fig19-swiss-army-knife.png')
knitr::include_graphics('./figures/lecture01-fig19-giant-swiss-army-knife.png')
```
...
- _**Simple scissor**_ -- an optimal tool for a special task.

- _**Swiss Army knife**_ -- a versatile tool that combines several individual functions in a single unit.

- _**Giant Swiss Army knife**_ -- a versatile tool that combines several individual functions in a single unit.
...
[8] Bretz F, Gallo P, Maurer W (2017)  "_Adaptive designs: The Swiss Army knife among clinical trial designs?_." _Clinical Trials_, **14**(5), 417–424.
...
tribble(
  ...
  row_spec(c(0, 2, 4, 6, 8), bold = TRUE, color = "white", background = params$blue_color) %>%
  row_spec(c(1, 3, 5, 7, 9), bold = TRUE, color = "white", background = params$green_color)
```
