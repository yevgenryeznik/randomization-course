...
```{r, layout="l-page",echo=FALSE, message=FALSE}
library(plotly)
a <- seq(1, 5, by = 0.5)
t <- seq(-5, 5, length = 100)
p <- purrr::map_df(a, ~ tibble::tibble(a = .x, t, y = 1/(1 + exp(-.x*t)))) %>% 
  ggplot(aes(t, y, frame = a))+
  geom_line()
plotly::ggplotly(p)
```
...
# Clinical Trial Design: Main Considerations (1 of 2)
...
# Clinical Trial Design: Main Considerations (2 of 2)

1. _**Study endpoints (outcome variables)**_
